...
class QuestionnaireRepository extends ServiceEntityRepository
{
    ...
    public function getReponsesOfUserForEachQuestionnaire(User $user): array
    {
        $tabReponsesUser = [];
        $questionnaires = $this->findAll();
        foreach ($questionnaires as $questionnaire) {
            $tabReponsesUser[$questionnaire->getId()] = [];
            foreach ($user->getReponsesOfAQuestionnaire($questionnaire->getId()) as $reponse) {
                $tabReponsesUser[$questionnaire->getId()][$reponse->getQuestion()->getNumero()] = $reponse;
            }
        }

        return [$tabReponsesUser, $questionnaires];
    }
